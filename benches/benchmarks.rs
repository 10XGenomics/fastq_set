...
fn simple_count(file: impl ToString) -> usize {
    let stdout = String::from_utf8(Command::new("wc")
        .arg("-l")
        .arg(file.to_string())
        .output()
        .unwrap()
...
fn lz4_count(file: impl ToString) -> usize {
    let stdout = String::from_utf8(Command::new("sh")
        .arg("lz4_wc.sh")
        .arg(file.to_string())
        .output()
        .unwrap()
...
fn run_fastq_lz4_benchmark(c: &mut Criterion) {
    c.bench_function("bench-lz4-wc", |b| b.iter(|| 
        assert_eq!(lz4_count(INTERLEAVED_LZ4_FASTQ), 16000) // 16000 lines
    ));
    c.bench_function("bench-lz4-read-pair-iter-count", |b| b.iter(|| 
        assert_eq!(read_pair_count(INTERLEAVED_LZ4_FASTQ), 2000) // 2000 read pairs
    ));
    let chemistry: ChemistryDef = serde_json::from_reader(File::open("tests/rna_read/sc_vdj_chemistry.json").unwrap()).unwrap();
    c.bench_function("bench-lz4-rna-chunk-iter-count", move |b| b.iter(|| 
        assert_eq!(rna_chunk_iter_count(chemistry.clone(), INTERLEAVED_LZ4_FASTQ), 2000) // 2000 read pairs
    ));
}

fn run_fastq_benchmark(c: &mut Criterion) {
    c.bench_function("bench-fastq-wc", |b| b.iter(|| 
        assert_eq!(simple_count(INTERLEAVED_FASTQ), 16000) // 16000 lines
    ));
    c.bench_function("bench-fastq-read-pair-iter-count", |b| b.iter(|| 
        assert_eq!(read_pair_count(INTERLEAVED_FASTQ), 2000) // 2000 read pairs
    ));
    let chemistry: ChemistryDef = serde_json::from_reader(File::open("tests/rna_read/sc_vdj_chemistry.json").unwrap()).unwrap();
    c.bench_function("bench-fastq-rna-chunk-iter-count", move |b| b.iter(|| 
        assert_eq!(rna_chunk_iter_count(chemistry.clone(), INTERLEAVED_FASTQ), 2000) // 2000 read pairs
    ));
}
